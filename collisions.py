...
def resolve_all_collisions(balls, holes, table_sides):
    ...
    for line_ball_combination in itertools.product(table_sides, balls):
        if physics.line_ball_combination_check(ball_hole_combination[0].line_ball_combination[1].ball):
            physics.collid_line_ball(line_ball_combination[0], line_ball_combination[1].ball)

    ball_list = balls.sprites()
    random.shuffle(ball_list)

    for ball_combination in itertools.combinations(ball_list, 2):
        if  physics.ball_combination_check(ball_combination[0].ball,ball_combination[1].ball):
            physics.collide_balls(ball_combination[0].ball, ball_combination[1].ball)
            zope.event.notify(event.GameEvent("POTTED", ball_combination))
