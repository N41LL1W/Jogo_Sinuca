...
def collide_balls(ball1, ball2):
    ...
    ball1.velocity += (ball2_dot - ball1_dot) * collision * 0.5 * (1+config.ball_coeff_of_restrition)
    ball2.velocity += (ball1_dot - ball2_dot) * collision * 0.5 * (1 + config.ball_coeff_of_restrition)
...
def line_ball_collision_check(line, ball):
    if distance_less_equal(line.middle, ball.pos, line.length / 2 + config.ball_radius):
        ...
        nomalised_point_diff_vector = displacement_to_second_point / \
